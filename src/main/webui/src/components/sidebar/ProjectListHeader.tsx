...
function ProjectListHeader({ createProject }: ProjectListHeaderProps) {
  ...
  return (
    <div className="flex flex-row text-base-content/70 text-sm items-center px-1 my-2">
      Projects
      <label htmlFor="new-project-modal-checkbox" className="ml-auto cursor-pointer stroke-base-content rounded-full hover:bg-base-200 p-0.5">
        <Plus className="ml-auto cursor-pointer rounded-full p-0.5" />
      </label>
      <input type="checkbox" id="new-project-modal-checkbox" className="peer sr-only" onChange={() => {
        const dialog: HTMLDialogElement | null = document.getElementById('new-project-modal') as HTMLDialogElement;
        dialog?.showModal();
      }} />
      <dialog id="new-project-modal" className="modal">
        <div className="modal-box">
          ...
          <div className="modal-action">
            <form method="dialog" className="w-full flex flex-col gap-4" onSubmit={onCreateProject}>
              <input name="projectName" required type="text" placeholder="Project Name" className="input input-bordered w-full mt-4" />
              ...
              <div className='flex flex-row justify-between items-center gap-4'>
                ...
                <input name="color" type="color" className="p-0 border-0 cursor-pointer" defaultValue="#99C1F1"/>
              </div>
