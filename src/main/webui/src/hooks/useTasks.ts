...
import { createTask, deleteTask, fetchAllTasks, fetchTask, updateTask } from "@/api/task";
...
export const useCreateTask = (projectId: Project["id"]) => {
  return useMutation({
    ...
    onSettled: (_data, _error, _variables, _onMutateResult, context) => {
      ...
      context.client.invalidateQueries({ queryKey: [TASKS_QUERY_KEY, projectId] });
    }
  });
}
...
export const useUpdateTask = (projectId: Project["id"]) => {
  return useMutation({
    mutationFn: ({ taskId, updatedTask }: { taskId: Task["id"], updatedTask: Partial<Omit<Task, "id" | "projectId">> }) => updateTask(projectId, taskId, updatedTask),
    onSettled: (_data, _error, variables, _onMutateResult, context) => {
      // Invalidate and refetch
      context.client.invalidateQueries({ queryKey: [TASKS_QUERY_KEY, projectId, variables.taskId] });
    }
  });
}
...
export const useDeleteTask = (projectId: Project["id"]) => {
  return useMutation({
    ...
    onSettled: (_data, _error, taskId, _onMutateResult, context) => {
      // Invalidate and refetch
      context.client.invalidateQueries({ queryKey: [TASKS_QUERY_KEY, projectId, taskId] });
    }
  });
}
